...
class FeedbackController {
  ...
  async create(req, res, next) {
    try {
      ...
      const participation = await Enrollment.findOne({
        where: {
          user_id: req.userId,
          event_id: eventoId,
  ...
  async update(req, res, next) {
    try {
      ...
      if (feedback.usuario_id !== req.userId) {
        return res.status(403).json({ success: false, message: 'Você não pode editar esta avaliação' });
      }
  ...
  async destroy(req, res, next) {
    try {
      ...
      // Verificar se é o dono do feedback
      if (feedback.usuario_id !== req.userId) {
        return res.status(403).json({ success: false, message: 'Você não pode deletar esta avaliação' });
      }
