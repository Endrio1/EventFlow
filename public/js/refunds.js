// Módulo de Reembolsos para o Dashboard
function initRefunds() {
  const container = document.getElementById('refundsContainer');
  
  if (!container) return; // Container não existe (não estamos na página dashboard)
  ...
  window.addEventListener('loadRefunds', loadRefunds);
}

// Inicializar quando o DOM estiver pronto
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initRefunds);
} else {
  initRefunds();
}