...
class EventsManager {
      ...
      <div class="event-card" data-event-id="${eventId}">
        ...
        <div class="event-content">
          ...
          <div class="event-actions">
            <button class="btn btn-primary btn-view-event" data-event-id="${eventId}">
              Ver Detalhes
            </button>
            ${event.sales_closed ? `
            ...
            ` : !isFull ? `
              <button class="btn btn-success btn-enroll-event" data-event-id="${eventId}">
                Inscrever-se
              </button>
            ` : `
              <button class="btn btn-secondary" disabled>
  ...
  async showEventDetails(eventId) {
    try {
      ...
      const eventIdInternal = event._id || event.id || eventId;
      ...
      const content = `
        ...
        <div class="event-details" style="margin-bottom: 1.5rem;">
          ...
          <div class="event-detail-item">
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <span><strong>Organizador:</strong> ${event.organizer?.name || 'Organizador'}</span>
          </div>
        </div>
        ...
        <hr />
          <section id="feedbackSection-${eventIdInternal}">
            <h4>Avaliações</h4>
            <div id="feedbackList-${eventIdInternal}">Carregando avaliações...</div>

            <div id="feedbackFormContainer-${eventIdInternal}">
              ${api.isAuthenticated() ? `
                <form id="feedbackForm-${eventIdInternal}" class="feedback-form" style="margin-top: 1rem; display: none;">
                  <input type="hidden" id="feedbackId-${eventIdInternal}" value="">
                  <label for="feedbackNota-${eventIdInternal}">Nota:</label>
                  <select id="feedbackNota-${eventIdInternal}" name="nota" required>
                    <option value="">--</option>
                  ...
                  <div style="margin-top: .5rem;">
                    <textarea id="feedbackComentario-${eventIdInternal}" name="comentario" placeholder="Deixe seu comentário (opcional)" rows="3" style="width:100%;"></textarea>
                  </div>
                  <div style="margin-top: .5rem; display: flex; gap: 0.5rem; justify-content: flex-end;">
                    ...
                    <button class="btn btn-secondary" type="button" id="cancelEditBtn-${eventIdInternal}" style="display: none;">Cancelar</button>
                  </div>
                </form>
      ...
      if (window.authManager) {
        ...
        (async () => {
          try {
            const listContainer = document.getElementById(`feedbackList-${eventIdInternal}`);
            const form = document.getElementById(`feedbackForm-${eventIdInternal}`);
            const formContainer = document.getElementById(`feedbackFormContainer-${eventIdInternal}`);
            ...
            if (api.isAuthenticated()) {
              try {
                ...
                isParticipant = enrollments.some(
                  e => e.event_id === eventIdInternal && (e.status === 'confirmed' || e.status === 'attended')
                );
              } catch (err) {
            ...
            // Carregar feedbacks
            const resp = await api.getFeedbacks(eventIdInternal);
            let feedbacks = [];
            if (resp && resp.data) {
              ...
              listContainer.querySelectorAll('.btn-edit').forEach(btn => {
                btn.addEventListener('click', () => {
                  ...
                  if (form) {
                    ...
                    form.style.display = 'block';
                    document.getElementById(`feedbackId-${eventIdInternal}`).value = feedbackId;
                    document.getElementById(`feedbackNota-${eventIdInternal}`).value = nota;
                    document.getElementById(`feedbackComentario-${eventIdInternal}`).value = comentario;
                    const cancelBtnEl = document.getElementById(`cancelEditBtn-${eventIdInternal}`);
                    if (cancelBtnEl) cancelBtnEl.style.display = 'inline-block';
                    // focar no select de nota para melhor UX
                    const notaEl = document.getElementById(`feedbackNota-${eventIdInternal}`);
              ...
              listContainer.querySelectorAll('.btn-delete').forEach(btn => {
                btn.addEventListener('click', async () => {
                  ...
                  try {
                    ...
                    // Recarregar lista
                    const updated = await api.getFeedbacks(eventIdInternal);
                    let fb = [];
            ...
            if (form) {
              const cancelBtn = document.getElementById(`cancelEditBtn-${eventIdInternal}`);
              if (cancelBtn) {
                cancelBtn.addEventListener('click', () => {
                  document.getElementById(`feedbackId-${eventIdInternal}`).value = '';
                  document.getElementById(`feedbackNota-${eventIdInternal}`).value = '';
                  document.getElementById(`feedbackComentario-${eventIdInternal}`).value = '';
              ...
              form.addEventListener('submit', async (e) => {
                e.preventDefault();
                const feedbackIdEl = document.getElementById(`feedbackId-${eventIdInternal}`);
                const notaEl = document.getElementById(`feedbackNota-${eventIdInternal}`);
                const comentarioEl = document.getElementById(`feedbackComentario-${eventIdInternal}`);
                ...
                try {
                  if (feedbackId) {
                    ...
                    // Criar
                    await api.createFeedback(eventIdInternal, { nota, comentario });
